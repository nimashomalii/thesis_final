...
def train(model, train_loader, test_loader, cfg, device, majority: float = 0.0):
    ...
    # ------------------------------------------------------------------
    # Full metric report (confusion matrix, F1, precision/recall, ROC-AUC)
    # on the test set. Uses the saved best checkpoint if SAVE_BEST is on,
    # otherwise the model's final-epoch weights.
    if cfg.SAVE_BEST:
        best_path = os.path.join(cfg.OUTPUT_DIR, f"best_{cfg.run_name()}.pt")
        if os.path.exists(best_path):
            model.load_state_dict(torch.load(best_path, map_location=device))

    final_metrics = compute_metrics(model, test_loader, device)
    print_metrics(final_metrics, title=f"FINAL TEST METRICS  ({cfg.run_name()})")
    hist["final_metrics"] = final_metrics
